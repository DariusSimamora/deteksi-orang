...
def main(video_path, model, tracker):
    ...
    while True:
        ...
        bboxes, confidences, class_ids = model.detect(image)  # people detection      
        tracks = tracker.update(bboxes, confidences, class_ids) # update tracker       
        updated_image = model.draw_bboxes(image.copy(), bboxes, confidences, class_ids) # draw boundingbox    
        updated_image = draw_tracks(updated_image, tracks) # draw centertroid      
        cv2.line(updated_image, (W // 2, 0), (W // 2, H), (0, 0, 255), 2) # draw center line
 
        # update monitor result
        for track in tracks:
            
            track_id = track[1] # track_id            
            x_coord = track[2] # x coordinate          
            width = track[4] # width          
            x_c = int(x_coord + 0.5 * width) # x_center  
            pts[track_id].append(x_c) # append x coordinate
