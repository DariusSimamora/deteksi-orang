...
def main():
    ...
    interpreter = tf.lite.Interpreter(model_path="./assets/ssd_mobilenet_v2_coco_quant_postprocess.tflite")
    ...
    input_mean = 127.5
    input_std = 127.5
    labels_path = os.path.join('./assets/coco_labels.txt')
    with open(labels_path, 'r') as f:
        lines = f.readlines()
    labels = [x.strip() for x in lines]
    #print('class names: {}'.format(labels))
    cap = cv2.VideoCapture(1)
    #cap = cv2.VideoCapture('/assets/pep.mp4')
    totalLeft = 0
    totalRight = 0
